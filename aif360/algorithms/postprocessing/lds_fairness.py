...
class _BaseLDS(Transformer):
    ...
    def fit(self, dataset):
        ...
        self.base_rate =  self._calculate_base_rate(D)
    ...
    def _calculate_base_rate(self, dataset):
        ...
        base_rate_privileged = 1 - D_privileged[D_privileged[self.Y_hat] == 1].shape[0] / D_privileged.shape[0]
        base_rate_unprivileged = 1 - D_unprivileged[D_unprivileged[self.Y_hat] == 1].shape[0] / D_unprivileged.shape[0]
...
class InstantaneousFairOptimiser(_BaseLDS):
    ...
    def _get_constraints(self, D_privileged, D_unprivileged, x_vars, e, z):
        ...
        for i in range(len(D_unprivileged)):
            x_i = D_unprivileged.iloc[i][self.X]
            y_hat_i = D_unprivileged.iloc[i][self.Y_hat]
            constraint1 = (z[0] + y_hat_i - sum(x_vars[j][0] * x_i[j] for j in range(len(self.X))) + e[0]) / len(D_unprivileged) >= 0
            constraint2 = (z[0] - y_hat_i + sum(x_vars[j][0] * x_i[j] for j in range(len(self.X))) + e[0]) / len(D_unprivileged) >= 0
            constraints.extend([constraint1, constraint2])

        for i in range(len(D_privileged)):
            x_i = D_privileged.iloc[i][self.X]
            y_hat_i = D_privileged.iloc[i][self.Y_hat]
            constraint3 = (z[0] + y_hat_i - sum(x_vars[j][1] * x_i[j] for j in range(len(self.X))) + e[1]) / len(D_privileged) >= 0
            constraint4 = (z[0] - y_hat_i + sum(x_vars[j][1] * x_i[j] for j in range(len(self.X))) + e[1]) / len(D_privileged) >= 0
            constraints.extend([constraint3, constraint4])

        # Constraint for z1
        z1_sum_unprivileged = 0
        for i in range(len(D_unprivileged)):
            y_hat_i = D_unprivileged.iloc[i][self.Y_hat]
            x_sum = sum(x_vars[j][0] * D_unprivileged.iloc[i][x] for j, x in enumerate(self.X))
            z1_sum_unprivileged += (z[1] + y_hat_i - x_sum + e[0]) / len(D_unprivileged)
        z1_constraint_unprivileged = z1_sum_unprivileged >= 0
        constraints.append(z1_constraint_unprivileged)

        z1_sum_privileged = 0
        for i in range(len(D_privileged)):
            y_hat_i = D_privileged.iloc[i][self.Y_hat]
            x_sum = sum(x_vars[j][1] * D_privileged.iloc[i][x] for j, x in enumerate(self.X))
            z1_sum_privileged += (z[1] - y_hat_i + x_sum - e[1]) / len(D_privileged)
        z1_constraint_privileged = z1_sum_privileged >= 0
        constraints.append(z1_constraint_privileged)

        # Objective function
        obj_D = z[0] + z[1] #+ 0.05*(e[0]**2 + e[1]**2)
