...
class InstantaneousFairness(Transformer):
    ...
    def _create_inequality(self, dataset_true, attr1, attr2, attr3):
        ...
        # Constraints
        ine1 = [(z[0]+Dtrain.loc[i ,self.predictCol] - a[0]*int(Dtrain.loc[i,attr1]<25) - c[0]*Dtrain.loc[i,attr2] - d[0]*Dtrain.loc[i,attr3] + e[0])/len(I0_train) for i in I0_train]
        ine2 = [(z[0]-Dtrain.loc[i ,self.predictCol] + a[0]*int(Dtrain.loc[i,attr1]<25) + c[0]*Dtrain.loc[i,attr2] + d[0]*Dtrain.loc[i,attr3] + e[0])/len(I0_train) for i in I0_train]
        ine3 = [(z[0]+Dtrain.loc[i ,self.predictCol] - a[1]*int(Dtrain.loc[i,attr1]<25) - c[1]*Dtrain.loc[i,attr2] - d[1]*Dtrain.loc[i,attr3] + e[1])/len(I1_train) for i in I1_train]
        ine4 = [(z[0]-Dtrain.loc[i ,self.predictCol] + a[1]*int(Dtrain.loc[i,attr1]<25) + c[1]*Dtrain.loc[i,attr2] + d[1]*Dtrain.loc[i,attr3] + e[1])/len(I1_train) for i in I1_train]
        max1 = [(z[1]+(Dtrain.loc[i ,self.predictCol]-a[0]*int(Dtrain.loc[i,attr1]<25) - c[0]*Dtrain.loc[i,attr2] - d[0]*Dtrain.loc[i,attr3] + e[0]))/len(I0_train) for i in I0_train]
        max2 = [(z[1]-(Dtrain.loc[i ,self.predictCol]-a[1]*int(Dtrain.loc[i,attr1]<25) - c[1]*Dtrain.loc[i,attr2] - d[1]*Dtrain.loc[i,attr3] + e[1]))/len(I1_train) for i in I1_train]
