...
class DistributionalRepair(Transformer):
    ...
    def fit(self, dataset_R):
        """Fit the Distributional Repair transformer to the reference dataset.

        Args:
            dataset_R (StandardDataset): Reference dataset to fit the transformer.
    ...
    def _continuous_fit(self, feat, u_val):
        """Fit the transformer for a continuous feature.
    ...
    def _discrete_fit(self, feat, u_val):
        """Fit the transformer for a discrete feature.
    ...
    def transform(self, dataset_D):
        """Transform the dataset to mitigate bias.
    ...
    def _get_support(self, feat, u_val):
        """Get the support for a continuous feature.
    ...
    def _get_pmfs(self, feat, u_val):
        """Get the probability mass functions for a continuous feature.
        ...
        Raises:
            ZeroDivisionError: If one or more PMFs have sum zero.
        """
    ...
    def _get_barycenter(self, pmf_0, pmf_1, feat, u_val):
        """Get the barycenter of the PMFs.
        ...
        Raises:
            RuntimeError: If no valid barycenter is found.
        """
    ...
    def _get_transport_plans(self, pmf_0, pmf_1, barycenter, feat, u_val):
        """Get the transport plans between the PMFs and the barycenter.
    ...
    def _get_discrete_pmfs(self, feat, u_val):
        """Get the PMFs for a discrete feature.
    ...
    def _get_discrete_transport_plan(self, pmf_0, pmf_1):
        """Get the transport plan between the PMFs of a discrete feature.

        Args:
            pmf_0 (Series): PMF for the protected group.
            pmf_1 (Series): PMF for the unprotected group.

        Returns:
            ndarray: Transport plan between the PMFs.
        """
    ...
    def _continuous_transform(self, s_D, u_D, x_D, feat, tilde_x_D, u_val, support, T_0, T_1):
        """Transform a continuous feature.
    ...
    def _repair_data(self, x, support_i, support_j, T, i_split=True, j_split=False):
        """
        Repairs the given data point `x` using the distributional repair algorithm.

        Args:
            x (float): The data point to be repaired.
            support_i (ndarray): The support of the input distribution.
            support_j (ndarray): The support of the target distribution.
            T (ndarray): The transport matrix representing the joint distribution.
            i_split (bool, optional): Whether to split the input distribution. Defaults to True.
            j_split (bool, optional): Whether to split the target distribution. Defaults to False.

        Returns:
            float: The repaired data point.
